...
@Component({
    selector: 'app-situacao-agendamentos',
    templateUrl: './situacao-agendamentos.component.html',
    styleUrls: ['./situacao-agendamentos.component.scss']
})
export class SituacaoAgendamentosComponent implements OnInit {

    tela: string;
    prestadoras: SelectItem[] = [];
    situacoesVistoria: SelectItem[] = [];
    relatorioAgendamento: RelatorioAgendamento[];
    prestadoraStatusDTO: any;
    ...
    constructor(
        ...
        private datepipe: DatePipe) {
            ...
            this.tela = "situacao-agendamentos"
            this.filter.formaAgrupamento = "P"
    }
    ...
    ngOnInit() {
        ...
        this.relatorioAgendamentoService.getSituacoesVistoria(this.tela).subscribe(
            response => {
    ...
    onPesquisar() {
        ...
        this.prestadoraStatusDTO = null;
        this.relatorioAgendamento = null;
        
        if(this.filter.formaAgrupamento == 'V') {
            this.relatorioAgendamentoService.getRelatorioSituacaoAgendamentosEager(
            ...
            this.relatorioAgendamentoService.getRelatorioStatusPrestadora(
                this.filter.numVoucher != null ? this.filter.numVoucher : '',
                this.filter.codCorretor != null ? this.filter.codCorretor : 0,
                this.filter.idPrestadora != null ? this.filter.idPrestadora : 0,
                this.filter.codSitVistoria != null ? this.filter.codSitVistoria : '',
                ...
                response => {
                    this.prestadoraStatusDTO = response;
                    this.loading = false;
                },
                error => {
                    this.messageService.error('Servidor não encontrado!');
