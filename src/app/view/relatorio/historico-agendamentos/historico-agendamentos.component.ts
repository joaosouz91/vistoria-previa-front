...
export class HistoricoAgendamentosComponent implements OnInit {
    ...
    onPesquisar() {
        ...
        this.relatorioAgendamentoService.getRelatorioHistoricoAgendamentosEager(
    ...
    validateFilter(){
        ...
        if(Math.abs(totalDays) > 2 
            ...
            && (this.filter.numPlaca == null || this.filter.numPlaca == '')
            && (this.filter.codCorretor == null || this.filter.codCorretor == 0)){
            this.messageService.error('Para um período maior que dois (2) dias é necessário escolher um ou mais filtros: [Número da Placa], [Número do Voucher], [Código do Corretor].')
            return false;
        }
    ...
    createExportList() {
        return this.relatorioAgendamento.map(item => {
            return {
                ...
                'Cliente': item.nmClien,
                'Placa': item.cdPlacaVeicu,
                'Chassi': item.cdChassiVeicu,
                'Situação': item.dsSitucAgend,
                'Usuário Alteração': item.cdUsuroUltmaAlter,
                'Motivo': item.dsMotvSitucAgmto,
                'Obs.': item.tpVspre,
                'Tipo': item.cdPlacaVeicu,
                'Data Atualização': item.dtUltmaAlterStatus        
            }
        }) 
    }
    ...
    exportExcel() {
        ...
        this.saveAsExcelFile(excelBuffer, "relatorio_historico_agendamentos");
    }
